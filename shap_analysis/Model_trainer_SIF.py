"""
Script: This script trains the model for the SIF analysis and store the results in a parquet output format. 
...
for variable_to_drop in variable_groups:
    for months_to_analyse in month_groups:
        ...
        for year in years:
            try:
                Yeardata = xr.open_dataset(os.path.join(datapath, f'PreprocessedData/ERAFLUXSAT/{months_to_analyse}/{year}.nc'), chunks = 'auto' )
        ...
        dataset = dataset.dropna(how = 'any', subset = ['SIF', 'GPP'])
        dataset = dataset.drop(variable_to_drop, axis  =1)



        outputdir = f'/fmi/scratch/project_2005798/data/Modelfiles/ERAFLUXSAT/{months_to_analyse}/Dropped_{variable_to_drop}'
